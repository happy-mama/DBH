...
class DBH {
  ...
  log(str) {
    console.log(
      "\x1b[32m" + "[DBH]: " + "\x1b[0m" + "\x1b[1m" + str || "" + "\x1b[0m"
    );
  }
  ...
  init(DB, options, GEN) {
    return new Promise((result, reject) => {
      ...
      if (!(DB.database || DB.host || DB.password || DB.login)) {
      ...
      this.mongoose
        .connect(
          `mongodb://${DB.login}:${DB.password}@${DB.host}/${DB.database}?retryWrites=true`
        )
        .then(() => {
    ...
    if (x.DB) {
      ...
      if (this.temp.DBNames.includes(x.DB.name)) {
        this.error(
          "DBH.js:gen",
          x.name,
          `prop.DB.name has already created in another prop`
        );
      }
      ...
      if (!Object.values(x.DB.schema.data)) {
        return this.error(
          "DBH.js:gen",
          x.name,
          `prop needs DB.schema.data values`
        );
      }

      p.$schema = new this.mongoose.Schema(
        x.DB.schema.data,
        x.DB.schema.params
      );
        ...
        if (x.JWT.string) {
          if (typeof x.JWT.string != "string") {
            return this.error(
              "DBH.js:gen",
              x.name,
              "prop JWT.string is not a string"
            );
          }
      ...
      if (x.cache.use) {
        ...
        if (!x.cache.identification) {
          this.error(
            "DBH.js:gen",
            x.name,
            "prop cache.identification is empty"
          );
        }
        ...
        if (typeof x.cache.identification != "string") {
          this.error(
            "DBH.js:gen",
            x.name,
            "prop cache.identification is not a string"
          );
        }
            ...
            kParams.forEach((kParam) => {
              switch (kParam) {
                case "mainMethodType":
                    ...
                    if (!f.params.mainMethod) {
                      ...
                      if (!p[f.params.mainMethod]) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.mainMethod is not defined`
                        );
                      }

                      if (
                        this.config.gen.formats[f.type].params.mainMethodType !=
                        "any"
                      ) {
                        if (
                          x.format[f.params.mainMethod].type !=
                          this.config.gen.formats[f.type].params.mainMethodType
                        ) {
                          this.error(
                            "DBH.js:gen",
                            `${x.name}.${f.name}`,
                            `params.mainMethod is not allowed in this format`
                          );
                        }
                      }
                    }
                  ...
                  break;
                case "secondMethodType":
                  {
                    if (!f.params.secondMethod) {
                      if (
                        f.params[
                          this.config.gen.formats.putById.params
                            .secondMethodDependency
                        ]
                      ) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.${this.config.gen.formats.putById.params.secondMethodDependency} is required by "secondMethod" param`
                        );
                      }
                    } else {
                      if (
                        ...
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.secondMethod is required by "${this.config.gen.formats.putById.params.secondMethodDependency}" param`
                        );
                      } else {
                        if (!p[f.params.secondMethod]) {
                          this.error(
                            "DBH.js:gen",
                            `${x.name}.${f.name}`,
                            `params.secondMethod is not defined`
                          );
                        }

                        if (
                          this.config.gen.formats[f.type].params
                            .secondMethodType != "any"
                        ) {
                          if (
                            x.format[f.params.secondMethod].type !=
                            this.config.gen.formats[f.type].params
                              .secondMethodType
                          ) {
                            this.error(
                              "DBH.js:gen",
                              `${x.name}.${f.name}`,
                              `params.secondMethod is not allowed in this format`
                            );
                          }
                        }
                      }
                ...
                case "cacheMethodType":
                  {
                    if (!f.params.cacheMethod) {
                      this.error(
                        "DBH.js:gen",
                        `${x.name}.${f.name}`,
                        `params.cacheMethod is required`
                      );
                    } else {
                      if (!p[f.params.cacheMethod]) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.cacheMethod is not defined`
                        );
                      }

                      if (
                        x.format[f.params.cacheMethod].type !=
                        this.config.gen.formats[f.type].params.cacheMethodType
                      ) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.cacheMethod is not allowed in this format`
                        );
                      }
                    }
                  }
                ...
                case "getJWTMethodType":
                  {
                    if (!f.params.getJWTMethod) {
                      this.error(
                        "DBH.js:gen",
                        `${x.name}.${f.name}`,
                        `params.getJWTMethod is required`
                      );
                    } else {
                      if (!p[f.params.getJWTMethod]) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.getJWTMethod is not defined`
                        );
                      }

                      if (
                        x.format[f.params.getJWTMethod].type !=
                        this.config.gen.formats[f.type].params.getJWTMethodType
                      ) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.getJWTMethodType is not allowed in this format`
                        );
                      }
                    }
                  }
                ...
                case "postJWTMethodType":
                  {
                    if (!f.params.postJWTMethod) {
                      this.error(
                        "DBH.js:gen",
                        `${x.name}.${f.name}`,
                        `params.postJWTMethod is required`
                      );
                    } else {
                      if (!p[f.params.postJWTMethod]) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.postJWTMethod is not defined`
                        );
                      }

                      if (
                        x.format[f.params.postJWTMethod].type !=
                        this.config.gen.formats[f.type].params.postJWTMethodType
                      ) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.postJWTMethodType is not allowed in this format`
                        );
                      }
                    }
                  }
                    ...
                    if (f.params.checkEquals) {
                      if (!Array.isArray(f.params.checkEquals)) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.checkEquals is not an Array`
                        );
                      }
                    }
                  }
                    ...
                    if (f.params.checkMethod) {
                      if (!p[f.params.checkMethod]) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.checkMethod is not defined`
                        );
                      }

                      if (
                        x.format[f.params.checkMethod].type !=
                        this.config.gen.formats[f.type].params.checkMethodType
                      ) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          `params.checkMethodType is not allowed in this format`
                        );
                      }
                    }
                  }
                ...
                case "disableCopies":
                  {
                    if (f.params.disableCopies) {
                      if (
                        !this.config.gen.formats[f.type].params.disableCopies
                      ) {
                        this.warn(
                          `${x.name}.${f.name}`,
                          `params.disableCopies is not allowed in this format`
                        );
                      }
                    }
                  }
                    ...
                    if (f.params.rules) {
                      ...
                      if (f.params.rules.length == 0) {
                        this.error(
                          "DBH.js:gen",
                          `${x.name}.${f.name}`,
                          "params.rules syntax error, rules must be set or not write this option"
                        );
                      }
                        ...
                        if (rule.includes("|")) {
                          if (rule.split("|").length <= 1) {
                            this.error(
                              "DBH.js:gen",
                              `${x.name}.${f.name}`,
                              `params.rules syntax error, rule "|" must be like [option1|option2|option3...]`
                            );
                          }
                          ...
                          splitRules.forEach((splitRule) => {
                            if (!x.DB.schema.data[splitRule]) {
                              this.error(
                                "DBH.js:gen",
                                `${x.name}.${f.name}``params.rules[${rule}] is not defined in prop schema`
                              );
                            }
                          });
                        } else {
          ...
          if (kErrors) {
            kErrors.forEach((kError) => {
              if (this.config.gen.formats[f.type].errors[kError]) {
                if (
                  typeof f.errors[kError] !=
                  this.config.gen.formats[f.type].errors[kError]
                ) {
                  this.error(
                    "DBH.js:gen",
                    `${x.name}.${f.name}`,
                    `typeof errors.${kError} != ${
                      config.gen.formats[f.type].errors[kError]
                    }`
                  );
                }
              } else {
                this.warn(
                  `${x.name}.${f.name}`,
                  `errors.${kError} is not using in this method`
                );
              }
            });
          }
